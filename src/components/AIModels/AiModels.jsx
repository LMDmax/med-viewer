...
const AiModels = ({
	...
	const handleKI67 = () => {
		...
		if (
			slide?.stainType !== "IHC" ||
			(slide?.bioMarkerType !== "kI67" && application === "hospital")
		) {
		...
		if (application === "education" && bottomZoomValue > 39) {
			setModelname("KI67");
		}
	};
	...
	const handleMorphometry = () => {
		...
		if (slide.stainType !== "H&E" && application === "hospital") {
			setToolSelected("MorphometrySlideIssue");
		}
		if (bottomZoomValue < 40) {
			setToolSelected("ZoomError");
		}
		if (application === "education" && bottomZoomValue >= 40) {
			setModelname("Morphometry");
		}
	};
