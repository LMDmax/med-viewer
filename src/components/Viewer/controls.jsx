...
const ViewerControls = ({
  ...
  // ############### LOAD_ANNOTATION ####################
  const { getAnnotation, annotationData, loading, error } = getAnnotationFromDB;
  const { getXmlAnnotation, xmlAnnotationData } = getXMLAnnotationFromDB;

  //################## GET_FILTER_DATA #################
  const { onGetFilterData, filterResponseData, filterResponseError } =
    getFilterDataFromDB;

  // ########## RUN MORPHOMETRY ##########################
  const { onVhutAnalysis, analysisData, analysisError } = onUpdateVhutAnalysis;

  // ######## SHOW ANALYSIS AFTER RUNNING MORPHOMETRY ##############
  const { onGetVhutAnalysis, responseData, responseError } =
    onGetVhutAnalysisData;

  // ######## UPDATE ANNOTATION ##############
  const { modifyAnnotation, updatedData, updateError } = onModifyAnnotation;
  ...
  const { subscriptionData, subscriptionError } =
    getAnnotationSubscriptionData(slideId);
  // #################### VHUT_ANALYSIS_SUBSCRIPTION ##############
  const { vhutSubscriptionData, vhutSubscriptionError } =
    getVhutSubscriptionData(slideId);
  ...
  useEffect(() => {
    // console.log(vhutSubscriptionData)
    if (vhutSubscriptionData !== undefined) {
      const {
        data,
        message,
        analysisType: type,
